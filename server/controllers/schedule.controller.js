...
import { getEmptySeatArray } from '../utils/seats';
...
export const storeSeats = async (req, res) => {
  try {
    const seats = getEmptySeatArray();

    await firebaseDb.update({ [req.params.id]: seats });

    res.json({
      error: false,
      data: { message: 'Schedule seats added successfully.' },
    });
  } catch (error) {
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: error.message });
  }
